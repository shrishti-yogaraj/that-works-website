  ...
  return (
    <div className="uth-pack">
      <div className="uth-pack-head">
        <span className={`uth-pack-count uth-pack-count--${tone}`}>{pack.count}</span>
        <h3>{pack.name}</h3>
      </div>
      <ul className="uth-pack-list">
        ...
        {expanded &&
          pack.hidden.map((item) => (
            <li key={item} className="uth-pack-extra">
              {item}
            </li>
          ))}
      </ul>
    ...
    <>
      ...
      <div className="uth">
        ...
        <section className="uth-hero">
          <div className="uth-wrap uth-center">
            ...
            <div className="uth-actions uth-actions--center">
              <button
                className="btn-primary"
                onClick={() => openPopup("under-the-hood-fit")}
              >
                See if it's for you →
              </button>
              {RAZORPAY_URL && (
        ...
        {/* ── SUMMARY ── */}
        <section className="uth-summary">
          <div className="uth-wrap">
            <div className="uth-summary-grid">
              {summary.map((s) => (
                <div key={s.label} className={`uth-sum uth-sum--${s.tone}`}>
                  <span className="uth-sum-val">{s.value}</span>
                  <span className="uth-sum-label">{s.label}</span>
                  <span className="uth-sum-detail">{s.detail}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── THE PROBLEM ── */}
        <section className="uth-problem">
          <div className="uth-wrap">
            <div className="uth-tag uth-tag--yellow">Where you are</div>
            <h2>
              Everyone has a theory. Nobody has the <em>evidence</em>.
            </h2>
            <div className="uth-symptoms">
              {symptoms.map((s, i) => (
                <div key={s} className="uth-symptom">
                  <span className="uth-symptom-num">
        ...
        <section className="uth-deliverables">
          <div className="uth-wrap">
            <div className="uth-tag uth-tag--lavender">What lands on your desk</div>
            <h2>
        ...
        <section className="uth-scope">
          <div className="uth-wrap">
            <div className="uth-tag uth-tag--orange">What we open up</div>
            <h2>
              Six areas. One <em>machine</em>.
            </h2>
            <p className="uth-body">
              They're stacked, not separate. Break the foundation and everything
              above it shows up as a funnel problem.
            </p>
            <div className="uth-areas">
              {areas.map((a, i) => (
                <div key={a.name} className="uth-area">
                  <span className={`uth-area-num uth-area-num--${tones[i % 3]}`}>
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <h3>{a.name}</h3>
                  <p>{a.body}</p>
        ...
        <section className="uth-process">
          <div className="uth-wrap">
            <div className="uth-tag uth-tag--yellow">How the week runs</div>
            <h2>
            ...
            <ol className="uth-steps">
              {steps.map((s, i) => (
                <li
                  key={s.title}
                  className={`uth-step${i === steps.length - 1 ? " uth-step--last" : ""}`}
                >
                  <span className="uth-step-num">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div className="uth-step-text">
                    <div className="uth-step-when">{s.when}</div>
                    <h3>{s.title}</h3>
                    <p>{s.body}</p>
                  </div>
                </li>
        ...
        {/* ── HOW WE WORK ── */}
        <section className="uth-principles">
          <div className="uth-wrap">
            <h2>
              How we <em>work</em>.
            </h2>
            <div className="uth-principle-list">
              {principles.map((p, i) => (
                <div key={p.title} className={`uth-principle uth-principle--${tones[i % 3]}`}>
                  <span className="uth-principle-tag">{p.tag}</span>
                  <h3>{p.title}</h3>
                  <p>{p.body}</p>
                </div>
              ))}
        ...
        {/* ── SAMPLE ── */}
        <section className="uth-sample">
          <div className="uth-wrap uth-center">
            <div className="uth-tag uth-tag--lavender">Try before you buy</div>
            <h2>
              Want a <em>taste</em>?
            </h2>
            <p>
            ...
            <button
              className="btn-primary"
              onClick={() => openPopup("under-the-hood-sample", "sample")}
            >
              Send us your domain →
            </button>
          </div>
        ...
        {/* ── FAQ ── */}
        <section className="uth-faq">
          <div className="uth-wrap">
            <div className="uth-tag uth-tag--lavender">Before you ask</div>
            <h2>The practical bits.</h2>
            <div className="uth-faq-grid">
              {faqs.map((f) => (
                <div key={f.q} className="uth-faq-item">
                  <h3>{f.q}</h3>
                  <p>{f.a}</p>
                </div>
              ))}
